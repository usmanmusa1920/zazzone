    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if request.user.is_authenticated:
                ...
                messages.success(
                    request, f'Weldone {full_name}, your message has been sent. We will inbox you in your zazzone account')
                return redirect('about')
            else:
                form.save()
                full_name = form.cleaned_data.get('full_name')
                messages.success(
                    request, f'Weldone {full_name}, your message has been sent. We really appreciate')
                return redirect('about')
    else:
...
def signupView(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request, f'Welcome {first_name} {last_name}, your account has been created, you are ready to login!')
...
def sendFriendRequest(request, req_pk):
    ...
    if receiver in sender.friends.all():
        messages.success(
            request, f'You are already a friend with {receiver.first_name} {receiver.last_name}')
        ...
        if friend_req:
            messages.success(
                request, f'{receiver.first_name} already sent you friend request')
            return redirect(reverse('profile_user', kwargs={'username': receiver}))
        ...
        if created:
            messages.success(
                request, f'Friend request sent to {receiver.first_name} {receiver.last_name}')
            return redirect(reverse('profile_user', kwargs={'username': receiver}))
        else:
            messages.success(
                request, f'You already sent friend request to{receiver.first_name} {receiver.last_name}')
            return redirect(reverse('profile_user', kwargs={'username': receiver}))
...
def acceptFriendRequest(request, req_pk):
    ...
    if friend_request.receiver == request.user:
        ...
        messages.success(
            request, f'You and {friend_request.sender.first_name}  {friend_request.sender.last_name} are now friends')
    ...
    # Here, if the sender of the request is the current user, then it will cancel the request and delete it from our data base.
        ...
        friend_request.delete()
        messages.success(
            request, f'You cancel friend request,you sent to {friend_request.receiver.first_name}\'')
...
def declineRequest(request, req_pk):
    ...
    if friend_request.receiver == request.user:
        friend_request.delete()
        messages.success(
            request, f'You declined {friend_request.sender.first_name}\'s friend request')
        return redirect(reverse('profile_user', kwargs={'username': friend_request.sender}))
...
def removeFriend(request, user_pk):
    ...
    if user_remove in my_friends.all() and request.user in user_remove.friends.all():
        ...
        messages.success(
            request, f'You removed {user_remove.first_name} {user_remove.last_name} from your friend list')
...
class UserProfile(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self):
        num_info = phonenumbers.parse(
            str(self.request.user.phone_number), None)
        ro_number = phonenumbers.parse(
            str(self.request.user.phone_number), 'RO')
        ...
        inter_num = phonenumbers.format_number(
            num_info, phonenumbers.PhoneNumberFormat.INTERNATIONAL)
    ...
    def get_queryset(self):
        user = get_object_or_404(User, username=self.kwargs.get(
            'username'))  # OR user = self.request.user
...
def viewUserProfile(request, username):
    ...
    friend_req_to_accept = FriendRequest.objects.filter(
        sender=view_user, receiver=request.user)
    ...
    friend_req_to_cancel = FriendRequest.objects.filter(
        sender=request.user, receiver=view_user)
    ...
    view_user_last_follower = Board.objects.get(
        owner=view_user).followers.all().order_by('username').last()
...
def profileUpdate(request):
    ...
    if request.method == 'POST':
        ...
        p_form = acc_forms.ProfileUpdate(
...
def imageUpdate(request):
    ...
    if request.method == 'POST':
        form = acc_forms.ImageUpdate(
            request.POST, request.FILES, instance=request.user)
        r = request.user.image.path
        if form.is_valid():
            default_img_path = os.path.join(BASE_DIR, 'media/user.jpg')
...
def bannerUpdate(request):
    ...
    if request.method == 'POST':
        form = acc_forms.BannerUpdate(
            request.POST, request.FILES, instance=request.user.profile)
        r = request.user.profile.banner.path
        if form.is_valid():
            default_img_path = os.path.join(BASE_DIR, 'media/user_banner_default.jpg')
            if r != default_img_path:
                if os.path.exists(r):
                    os.remove(r)
            form.save()
