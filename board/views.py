...
def followBoard(request, user_id):
    ...
    if request.user not in board.followers.all():
        ...
        messages.success(
            request, f'You are now a follower in this board ({board.owner.first_name} {board.owner.last_name})')
        return redirect(reverse('board:board', kwargs={'board_id': board.id}))
    else:
        messages.success(
            request, f'You are already already a follower in this board ({board.owner.first_name} {board.owner.last_name})')
        return redirect(reverse('board:board', kwargs={'board_id': board.id}))
...
def unfollowBoard(request, user_id):
    ...
    if request.user not in board.followers.all():
        messages.success(
            request, f'You are not a follower in this board ({board.owner.first_name} {board.owner.first_name})')
        return redirect(reverse('board:board', kwargs={'board_id': board.id}))
    else:
        board.followers.remove(request.user.id)
        messages.success(request, f'You removed your self from ' +
                         str(board.owner.first_name) + ' board')
        return redirect(reverse('board:board', kwargs={'board_id': board.id}))
...
def boardImageUpdate(request):
    ...
    if request.method == 'POST':
        form = BoardImageUpdate(
            request.POST, request.FILES, instance=request.user.board_owner)
        r = request.user.board_owner.image.path
        print(r)
        if form.is_valid():
            default_img_path = os.path.join(BASE_DIR, 'media/board_default_img.jpg')
