...
def chat(request, username):
    ...
    msgs_filter = Message.objects.filter(
        to_receiver=request.user.id, is_msg_added=False, is_new_message=False).order_by('-timestamp') | Message.objects.filter(to_receiver=request.user.id, is_msg_added=True, is_msg_replied=False).order_by(
        '-timestamp') | Message.objects.filter(from_sender=request.user.id, is_msg_added=False, is_new_message=False).order_by('-timestamp') | Message.objects.filter(from_sender=request.user.id, is_msg_added=True, is_msg_replied=False).order_by('-timestamp')
    ...
    unread_msg = Message.objects.filter(
        to_receiver=request.user.id, is_view=False)
...
def sendMessage(request, user_id):
    ...
    try:
        is_seen = Message.objects.filter(
        ...
        if is_seen.to_receiver == request.user:
            ...
            is_msg_view = Message.objects.filter(
                to_receiver=request.user.id, from_sender=my_friend, is_view=False)
    ...
    msg = Message.objects.filter(Q(from_sender=request.user, to_receiver=my_friend) | Q(
        from_sender=my_friend, to_receiver=request.user)).all().order_by('-timestamp')
    ...
    if my_friend in request.user.friends.all():
            ...
            if m_form.is_valid():
                ...
                try:
                    ...
                    if get_msgs_1.from_sender == request.user:
                        ...
                        is_new_msg = Message.objects.filter(from_sender=request.user.id, to_receiver=my_friend, is_msg_added=False, is_new_message=False) | Message.objects.filter(
                            from_sender=request.user.id, to_receiver=my_friend, is_msg_added=True, is_new_message=False)
        ...
        for friend in FriendRequest.objects.all():
            if friend.sender == receiver and friend.receiver == sender:
                if receiver.gender == 'female':
                    messages.success(
                        request, f'You must accept {receiver.first_name} friend request in other to chat with her')
                    return redirect(reverse('search', kwargs={'username': sender}))
                else:
                    messages.success(
                        request, f'You must accept {receiver.first_name} friend request in other to chat with him')
        ...
        if created:
            if receiver.gender == 'female':
                messages.success(
                    request, f'{receiver.first_name} is not in your friend list, but friend request was sent to her')
            else:
                messages.success(
                    request, f'{receiver.first_name} is not in your friend list,  but friend request was sent to him')
            return redirect(reverse('profile_user', kwargs={'username': receiver}))
        else:
            if receiver.gender == 'female':
                messages.success(
                    request, f'{receiver.first_name} is not in your friend list, but friend request was already sent to her')
            else:
                messages.success(
                    request, f'{receiver.first_name} is not in your friend list, but friend request was already sent to him')
            return redirect(reverse('profile_user', kwargs={'username': receiver}))
        return False
