...
def searchView(request):
    ...
    try:
        ...
        # This is a filter that filter user matching a giving query.
        users_filter = User.objects.filter(Q(first_name=search_panel) | Q(last_name=search_panel) | Q(
    ...
    try:
        ...
        blog_posts_db = Blog_Post.objects.filter(Q(title__istartswith=search_panel) | Q(title__contains=search_panel) | Q(
            text__istartswith=search_panel) | Q(code__istartswith=search_panel)).order_by('-last_modified')
    except:
        # This is a filter that filter blog post matching a giving query.
        blog_posts_db = Blog_Post.objects.filter(
            Q(title=search_panel) | Q(text=search_panel) | Q(code=search_panel)).order_by('-last_modified')
    try:
        # This is a filter that filter board post matching a giving query.
        board_posts_db = Board_Post.objects.filter(
            Q(title__istartswith=search_panel) | Q(text__istartswith=search_panel) | Q(code__istartswith=search_panel)).order_by('-last_modified')
    except:
        # This is a filter that filter board post matching a giving query.
        board_posts_db = Board_Post.objects.filter(
            Q(title=search_panel) | Q(text=search_panel) | Q(code=search_panel)).order_by('-last_modified')
    ...
    try:
        ...
        zones_db = Zone.objects.filter(
            Q(name__istartswith=search_panel) | Q(description__istartswith=search_panel)).order_by('-timestamp')
    except:
        # This is a filter that filter zone matching a giving query.
        zones_db = Zone.objects.filter(
            Q(name=search_panel) | Q(description=search_panel)).order_by('-timestamp')
