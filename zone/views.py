...
class ZoneCreate(FormView):
  ...
  def form_valid(self, form):
    ...
    messages.success(self.request, f'Your just create a new zone ({name} zone)')
...
def updateZoneImage(request, zone_id):
  ...
  if request.user == zone.creator:
    if request.method == 'POST':
      ...
      r = zone.image.path
      if form.is_valid():
        if r != '/home/usman/Desktop/acode/sakyum/sakyum/media/zone_default_img.jpg':
          if os.path.exists(r):
            os.remove(r)
        form.save()
  ...
  if request.user == zone.creator:
    if r != '/home/usman/Desktop/acode/sakyum/sakyum/media/zone_default_img.jpg':
      if os.path.exists(r):
        os.remove(r)
    zone.delete()
    messages.success(request, f'You just deleted your zone (' + str(zone.name) + ')')
...
def zoneDetail(request, zone_id, username=False):
  ...
  if request.user in zone.members.all():
    ...
    messages.success(request, f'You are not a member, try to join the zone in other to send message in the zone')
    return render(request, 'zone/zone_detail.html', context)
...
def memberAdd(request, user_pk, id):
  ...
  if zone.creator == request.user:
    if add_member in zone.members.all():
      messages.success(request, f'{add_member.first_name} {add_member.last_name} was already a member in this zone')
      return redirect(reverse('zone_detail', kwargs={'zone_id':id, 'username': request.user}))
    else:
      zone.members.add(add_member.id)
      messages.success(request, f'You just added a new member in this zone ({add_member.first_name} {add_member.last_name})')
      return redirect(reverse('zone_detail', kwargs={'zone_id':id, 'username': request.user}))
  else:
    return False
        ...
        for member_msg in Message.objects.filter(zone=zone, zone_sender=remove_member):
          if member_msg.image:
            ...
            if os.path.exists(r):
              os.remove(r)
          member_msg.delete()
        messages.success(request, f'You just remove a member in this zone ({remove_member.first_name} {remove_member.last_name})')
        return redirect(reverse('zone_detail', kwargs={'zone_id':id, 'username': request.user}))
      else:
        messages.success(request, f'Zone admin can\'t remove him self from his zone ({remove_member.first_name} {remove_member.last_name})')
        return redirect(reverse('zone_detail', kwargs={'zone_id':id, 'username': request.user}))
    else:
      messages.success(request, f'{remove_member.first_name} {remove_member.last_name} is not a member in this zone ({zone.name})')
      return redirect(reverse('zone_detail', kwargs={'zone_id':id, 'username': request.user}))
  else:
    return False
    ...
    for member_msg in Message.objects.filter(zone=zone, zone_sender=request.user):
      if member_msg.image:
        ...
        if os.path.exists(r):
          os.remove(r)
      member_msg.delete()
    messages.success(request, f'You removed your self from ' + str(zone.name) + ' zone')
...
def joinZone(request, zone_id):
  ...
  if request.user not in zone.members.all():
    ...
    messages.success(request, f'You are now a member in this zone ({zone.name})')
    return redirect(reverse('zone_message', kwargs={'zone_id':zone_id}))
  else:
    messages.success(request, f'You are already already a member in this zone ({zone.name})')
    return redirect(reverse('zone_message', kwargs={'zone_id':zone_id}))